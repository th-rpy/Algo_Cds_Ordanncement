...
def Alg(R):
    ...
    Story.append(t)
    Story.append(Spacer(1, 12))
    ptext = 'Probleme d ordanncement Flow Shop : {0} Taches sur {1} Machines'.format(
        len(data)-1, len(data[0])-1)
    Story.append(Paragraph(ptext, styles["Normal"]))
    Story.append(Spacer(1, 12))
    ptext = 'On résoult les {0} Problemes d ordanncement (Virtuel) Flow Shop à 2 machines avec l algorithme de Johnson  '.format(
        len(data[0]) - 2)
    Story.append(Paragraph(ptext, styles["Normal"]))
    Story.append(Spacer(1, 12))
    ...
    for p in range(len(s)):
        ...
        print(
            '===>' + ' \033[1;41m Cmax(heurstiq)={} \033[1;m'.format(s[p][v - 1]))
    ...
    Story.append(Spacer(1, 12))
    ptext = "\n" + \
        "Pour Conclure On retient La solution de Sequence Optimal:|| {0} || et d'ordonnancement optimal de Cmax = {1} ".format(
            klks[0][0], klks[0][1][0])
    ptext = ptext.replace(' ', '&nbsp;')
    ptext = ptext.replace('\n', '<br />')
    ptext = ptext.replace('\t', '&nbsp;&nbsp;&nbsp;&nbsp;')
...
def callback():
    if askyesno('Question', 'Vous avez un document texte du donnes ?'):
        filename = askopenfile(title="Ouvrir votre document", filetypes=[('txt files', '.txt'), (
            'Pdf files', '.pdf'), ('Img File', '.png'), ('img File', '.jpeg'), ('img file', '.jpg')])
        rrf = filename.name
...
def sendd():
    ...
    for i in range(assd):
        msg['To'] = input(
            'Enter The List of Receiption : (exp : xyz@gmail.com,frf@gmail.com)')
        too.append(msg['To'])
    frm = msg['From']
    ...
    for i in too:
        j += 1
        barr()
        print("email N°{} to {} sent successfully".format(j, i))
